...
var build_base = function(data){
	...
	leaf_numbers.append('text').text('Species Described By Phylum')
		.attr('transform','rotate(-90),translate(-'+width/4+',15)')
		.attr('class','title');
...
var build_author = function(data){
	...
	for (var i = 0; i< author[0].years.length; i++){
		...
		if (unique_phylums.indexOf(author[0].years[i].phylum) == -1) {
			unique_phylums.push(author[0].years[i].phylum);
		}
...
var build_legend = function(){
	...
	legend.selectAll('.legend-rect').data(unique_phylums).enter()
		.append('rect')
		.attr({
				x: 0,
				y: function(d,i){ return space * i; },
				width: 10,
				height: 10,
				"class":'legend-rect'
...
var build_viz = function(){
	d3.json('js/tree-authors-utf8-10.json',function(data){
		...
		build_scales(half_data);
		build_base(half_data);
	
		for (var i =0; i<half_data.length; i++){
			build_author(half_data[i]);
		}
